...
All of the front end development was written using the WebStorm IDE with Google Chrome to run and debug the application. Writing the application in the form of a website made the experiment easy to share which reduced the friction gathering results can create. Another added benefit was the reduction in the chances of investigator/observer bias \cite{phillips1999double} to occur. The questionnaires that were asked after the creation of each level were embedded in the application


Git was used as the version control software, with TortoiseGit to provide a graphical interface. 

The level editor is on a website which hosted on Digital Ocean server that is running Ubuntu.  The server code is written in python3 using the HTTPServer module to handle the results sent by the the level editor. The Python3 module called TinyDB was used to store the any results. To deploy the sever onto the live build git hooks was used to checkout the master branch into the required repository. These modules performed unit tests and to check for styling mistakes. The code checks and unit tests were performed using the pre-commit git hook, if the results came back with a failure the commit would fail. 

\subsection{Software Testing}
To ensure a high standard of code Node.js was used to run JSHint/Jest modules.
