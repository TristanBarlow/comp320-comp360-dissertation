...
Prototyping a level should be a fast iterative process \cite{smith2011tanagra}, the method used to prototype should allow for instant feedback on the design. This feedback needs to have an easy channel back to the designer so that amendments can be made with easily. Map generation algorithms, even in under ideal circumstances will only provide the designer with a set of parameters to change\cite{doran2010controlled}. Small variations in the given parameters can produce large changes in the maps design\cite{regier2009random}. This will make it difficult to make small changes when given feedback. As a result, this reduces the effectiveness of a CAD approach when trying to prototype levels.
...
\item An increase into the number of clicks participants have to make.
\item An increase into the exploration of alternative level designs.
\end{itemize}
